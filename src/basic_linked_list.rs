...
impl<T: fmt::Debug> LinkedList<T> {
    ...
    */
    ...
    pub fn extend<I>(&mut self, values: I) -> Vec<usize>
    where
    ...
    ///
    /// Returns the indexes of the new items
    pub fn extend_front<I>(&mut self, values: I) -> Vec<usize>
    where
        I: IntoIterator<Item = T>,
    {
        let mut indexes = Vec::new();
    ...
    pub fn remove(&mut self, index: usize) -> T {
        ...
        if let Some(next) = item.next_index {
            self.items.get_mut(next).unwrap().prev_index = item.prev_index;
        } else {
...
mod tests {
    ...
    fn test_popback() {
        let mut list = LinkedList::new();
        let _verticies: Vec<usize> = (0..100)
            .map(|i| list.push_back(format!("Node: {}", i.to_string())))
            .collect();
